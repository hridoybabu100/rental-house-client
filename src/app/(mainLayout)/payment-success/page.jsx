...
export default async function PaymentSuccess({ searchParams }) {
    ...
    const paymentData = { amount: session?.metadata?.amount, evetId: session?.metadata?.eventId, eventTitle: session?.metadata?.eventTitle, quantity: session?.metadata?.quantity, email: session?.metadata?.email, paymentType: "booking", transactionId: session?.payment_intent?.id, paymentStatus: session?.payment_status }
